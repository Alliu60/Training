...
Tone: [Note the tone, such as enthusiastic, persuasive, etc.]`
  ...
  const categories = [
    { id: 'all', name: 'All Prompts', icon: BookOpen, count: prompts.length },
    { id: 'activities', name: 'Activities', icon: Users, count: prompts.filter(p => p.category === 'activities').length },
    { id: 'lesson-plans', name: 'Lesson Plans', icon: FileText, count: prompts.filter(p => p.category === 'lesson-plans').length },
    { id: 'differentiation', name: 'Differentiation', icon: Target, count: prompts.filter(p => p.category === 'differentiation').length },
    { id: 'creative-enhancements', name: 'Creative Enhancements', icon: Star, count: prompts.filter(p => p.category === 'creative-enhancements').length },
    { id: 'assessments', name: 'Assessments', icon: FileText, count: prompts.filter(p => p.category === 'assessments').length },
    { id: 'communication', name: 'Communication', icon: MessageSquare, count: prompts.filter(p => p.category === 'communication').length }
  ];
        ...
        <div className="max-w-7xl mx-auto px-4 py-6">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            <p className="text-blue-800 text-sm">
              <strong>86%</strong> of teachers expect to save at least 2 hours per week using generative AI tools after completing the course.
          ...
          <div className="flex-1">
            ...
            <div className="space-y-4">
              {filteredPrompts.map((prompt) => (
                <div key={prompt.id} className="bg-white rounded-lg shadow-sm border">
                  <div className="p-6">
                    ...
                    <div className="flex space-x-2">
                      ...
                      >
                        {expandedPrompt === prompt.id ? 'Hide Template' : 'View Template'}
                      </button>
                      <button
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">
              A two-hour, self-paced course designed to help teachers save time, enhance student learning, 
              and personalize instruction with generative AI tools.
            </p>
            <div className="flex flex-wrap justify-center gap-6 text-sm text-gray-500">
              <span>✓ Flexible - 100% online and about 2 hours long</span>
              <span>✓ Practical, tailored training designed specially for educators</span>
              <span>✓ No previous experience necessary</span>
              <span>✓ Developed by AI experts at Google in collaboration with MIT RAISE</span>
            </div>
          </div>
        </div>
      </div>
